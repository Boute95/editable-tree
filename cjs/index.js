...
const EditableTree = ({ tree, className, isFullyExpanded = true, contentEditNode, contentRemoveNode, }) => {
    ...
    const onClick = (node, path) => {
        ...
        const nodeToToggle = core.Tree.nodeFromPath(path, newTree); // Taking node input doesnt expand when isFullyExpanded=true
    ...
    return (React.createElement("div", { className: className !== null && className !== void 0 ? className : '' },
        React.createElement(core.Tree, { className: styles.tree, contents: treeData, onNodeClick: onClick, onNodeCollapse: onClick, onNodeExpand: onClick })));
};
///////////////////////////////////////////////////////////////////////////////
...
function stylizeTree(tree, contentEditNode, contentRemoveNode) {
    ...
    TreeUtils.walk(rootTree, (child) => {
        ...
        if (contentEditNode || contentRemoveNode) {
            node.secondaryLabel = (React.createElement(core.ButtonGroup, { minimal: true },
                contentEditNode && (React.createElement(core.Popover, { content: contentEditNode(node), popoverClassName: core.Classes.POPOVER_CONTENT_SIZING },
                    React.createElement(core.Button, { icon: "edit", onClick: (e) => (node.isExpanded = !node.isExpanded) }))),
                contentRemoveNode && (React.createElement(core.Popover, { content: contentRemoveNode(node), popoverClassName: core.Classes.POPOVER_CONTENT_SIZING },
                    React.createElement(core.Button, { icon: "cross", onClick: (e) => (node.isExpanded = !node.isExpanded) })))));
        }
    });
    return newTree;
}

exports.EditableTree = EditableTree;
